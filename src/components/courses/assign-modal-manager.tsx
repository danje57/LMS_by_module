...
export function AssignModalManager({ courseId, courseTitle, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-[#1C1C1E] rounded-2xl shadow-xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E5E5EA] dark:border-[#3A3A3C] shrink-0">
          <div>
            ...
            <p className="text-[12px] text-[#6E6E73] dark:text-[#8E8E93] mt-0.5 ml-7 line-clamp-1">{courseTitle}</p>
          </div>
        ...
        {callerTeams.length > 1 && (
          <div className="px-6 py-3 border-b border-[#E5E5EA] dark:border-[#3A3A3C] bg-amber-50 dark:bg-amber-500/10 shrink-0">
            <div className="flex items-center gap-3">
              <Building2 className="w-4 h-4 text-amber-600 shrink-0" />
              <div className="flex-1">
                <p className="text-[13px] font-medium text-amber-800 dark:text-amber-400">Contexte équipe</p>
                <p className="text-[12px] text-amber-600 dark:text-amber-400">Ces affectations seront rattachées à l&apos;équipe choisie.</p>
              </div>
              <select
                value={teamContext ?? ""}
                onChange={(e) => setTeamContext(e.target.value || null)}
                className="h-9 px-3 rounded-xl border border-amber-200 dark:border-amber-500/30 bg-white dark:bg-[#2C2C2E] text-[13px] text-[#1D1D1F] dark:text-[#F5F5F7] outline-none focus:border-[#0071E3] transition-all"
              >
          ...
          <>
            ...
            <div className="px-6 pt-4 shrink-0">
              <div className="flex items-center gap-3 p-3 bg-[#F5F5F7] dark:bg-[#2C2C2E] rounded-xl">
                <CalendarDays className="w-4 h-4 text-[#6E6E73] dark:text-[#8E8E93] shrink-0" />
                <span className="text-[13px] text-[#1D1D1F] dark:text-[#F5F5F7] font-medium flex-1">{t("deadline")}</span>
                <input
                  type="date"
                  value={commonDueDate}
                  min={new Date().toISOString().slice(0, 10)}
                  onChange={(e) => applyCommonDate(e.target.value)}
                  className="h-8 px-2 rounded-lg border border-[#D2D2D7] dark:border-[#3A3A3C] bg-white dark:bg-[#1C1C1E] text-[12px] dark:text-[#F5F5F7] outline-none focus:border-[#0071E3] transition-all"
                />
                {commonDueDate && (
                  <button
                    onClick={() => { setCommonDueDate(""); setDueDates({}); }}
                    className="text-[11px] text-[#ADADB8] hover:text-[#1D1D1F] dark:hover:text-[#F5F5F7] transition-colors"
                  >
                    Effacer
                  </button>
                )}
              ...
              {(["teams", "users"] as const).map((tabKey) => (
                <button key={tabKey} onClick={() => setTab(tabKey)}
                  className={cn("px-4 py-2 rounded-xl text-[13px] font-medium transition-all",
                    tab === tabKey ? "bg-[#0071E3] text-white" : "text-[#6E6E73] dark:text-[#8E8E93] hover:bg-[#F5F5F7] dark:hover:bg-[#2C2C2E]"
                  )}>
                  {tabKey === "teams" ? <><Users className="w-3.5 h-3.5 inline mr-1.5" />{t("teams")}</> : t("individual")}
                </button>
              ))}
              <div className="flex-1" />
              <span className="text-[12px] text-[#6E6E73] dark:text-[#8E8E93] self-center">{t("selectedUsers", { count: selected.size })}</span>
            </div>
              ...
              {tab === "teams" ? (
                teams
                  ...
                  .map((team) => {
                    ...
                    return (
                      <div key={team.id} className="rounded-xl border border-[#E5E5EA] dark:border-[#3A3A3C] overflow-hidden">
                        <button
                          onClick={() => toggleTeam(team)}
                          className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#F5F5F7] dark:hover:bg-[#2C2C2E] transition-colors text-left"
                        >
                          <div className={cn("w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-all",
                            allIn ? "bg-[#0071E3] border-[#0071E3]" : someIn ? "border-[#0071E3] bg-[#0071E3]/10" : "border-[#D2D2D7] dark:border-[#636366]"
                          )}>
                            {allIn && <CheckCheck className="w-3 h-3 text-white" />}
                          </div>
                          <Users className="w-4 h-4 text-[#6E6E73] dark:text-[#8E8E93]" />
                          <span className="text-[14px] font-medium text-[#1D1D1F] dark:text-[#F5F5F7] flex-1">{team.name}</span>
                          <span className="text-[12px] text-[#ADADB8]">{teamUsers.length} membres</span>
                        </button>
                        {teamUsers.map((u) => (
                          <label key={u.id} className="flex items-center gap-3 px-4 py-2 bg-[#FAFAFA] dark:bg-[#2C2C2E] border-t border-[#F2F2F7] dark:border-[#3A3A3C] cursor-pointer hover:bg-[#F5F5F7] dark:hover:bg-[#1C1C1E] transition-colors">
                            <input type="checkbox" checked={selected.has(u.id)} onChange={() => toggleUser(u.id)}
                              className="w-4 h-4 rounded accent-[#0071E3]" />
                            <span className="text-[13px] text-[#1D1D1F] dark:text-[#F5F5F7] flex-1">{label(u)}</span>
                            {alreadyAssigned.has(u.id) && (
                              <span className="text-[10px] font-medium text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded-md">Déjà assigné</span>
                            )}
                ...
                filteredUsers.map((u) => (
                  <label key={u.id} className={cn(
                    ...
                    alreadyAssigned.has(u.id) ? "bg-emerald-50/50 dark:bg-emerald-500/10 hover:bg-emerald-50 dark:hover:bg-emerald-500/20" : "hover:bg-[#F5F5F7] dark:hover:bg-[#2C2C2E]"
                  )}>
                    <input type="checkbox" checked={selected.has(u.id)} onChange={() => toggleUser(u.id)}
                      className="w-4 h-4 rounded accent-[#0071E3]" />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-[13px] font-medium text-[#1D1D1F] dark:text-[#F5F5F7]">{label(u)}</p>
                        {alreadyAssigned.has(u.id) && (
                          <span className="text-[10px] font-medium text-emerald-600 bg-emerald-100 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded-md shrink-0">Déjà assigné</span>
                        )}
                      </div>
                    ...
                    {selected.has(u.id) && (
                      <input
                        ...
                        className={cn(
                          ...
                          alreadyAssigned.has(u.id)
                            ? "border-emerald-200 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10 focus:border-emerald-400"
                            : "border-[#D2D2D7] dark:border-[#3A3A3C] bg-white dark:bg-[#2C2C2E] focus:border-[#0071E3] dark:text-[#F5F5F7]"
                        )}
                      />
                    )}
                  </label>
        ...
        {/* Footer */}
        <div className="flex gap-3 px-6 py-4 border-t border-[#E5E5EA] dark:border-[#3A3A3C] shrink-0">
