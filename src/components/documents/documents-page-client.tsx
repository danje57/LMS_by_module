...
import type { DocumentRow } from "@/components/documents/document-admin-client";

// ── Constants ─────────────────────────────────────────────────────────────────

const PAGE_SIZES = [0, 5, 10, 20, 50] as const;

type MineStatus  = "not_started" | "in_progress" | "signed";
type MineFilter  = "all" | MineStatus;
type ActiveTab   = "mine" | "library";

const MINE_GROUPS: { key: MineStatus; label: string; color: string }[] = [
...
function UploadModal({ isAdmin, onClose, onSuccess }: { isAdmin: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isAdmin && department.trim()) form.append("category", department.trim());
          ...
          <div>
            ...
            <label className={`flex items-center gap-3 cursor-pointer border-2 border-dashed rounded-xl px-4 py-5 transition-colors ${file ? "border-[#0071E3] bg-blue-50 dark:bg-blue-500/10" : "border-[#D2D2D7] dark:border-[#3A3A3C] hover:border-[#0071E3]"}`}>
              ...
              <div className="flex-1 min-w-0">
                <span className="text-[13px] text-[#6E6E73] dark:text-[#8E8E93] truncate block">
...
function EditDocumentModal({ doc, isAdmin, onClose, onSuccess }: {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isAdmin) form.append("category", department.trim());
    if (file) form.append("file", file);
    const res  = await fetch(`/api/admin/documents/${doc.id}`, { method: "PATCH", body: form });
    const data = await res.json();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white dark:bg-[#1C1C1E] rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-[18px] font-semibold text-[#1D1D1F] dark:text-[#F5F5F7]">Modifier le document</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-[#F5F5F7] dark:hover:bg-[#2C2C2E] transition-colors">
            <X className="w-4 h-4 text-[#6E6E73]" />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-[13px] font-medium text-[#1D1D1F] dark:text-[#F5F5F7] mb-1.5">
              Remplacer le fichier PDF <span className="text-[#ADADB8] font-normal">(optionnel)</span>
            </label>
            <label className={`flex items-center gap-3 cursor-pointer border-2 border-dashed rounded-xl px-4 py-4 transition-colors ${file ? "border-[#0071E3] bg-blue-50 dark:bg-blue-500/10" : "border-[#D2D2D7] dark:border-[#3A3A3C] hover:border-[#0071E3]"}`}>
              <FileCheck className={`w-5 h-5 shrink-0 ${file ? "text-[#0071E3]" : "text-[#ADADB8]"}`} />
              <div className="flex-1 min-w-0">
                <span className="text-[13px] text-[#6E6E73] dark:text-[#8E8E93] truncate block">
                  {file ? file.name : doc.originalFileName || "Cliquez pour remplacer le PDF actuel"}
                </span>
                {file && <span className="text-[11px] text-[#ADADB8]">{formatSize(file.size)}</span>}
              </div>
            ...
            <p className="text-[11px] text-[#ADADB8] mt-1">Laissez vide pour conserver le fichier actuel · Max 100 Mo</p>
          </div>
          {error && <p className="text-[13px] text-red-600">{error}</p>}
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 h-10 rounded-xl border border-[#D2D2D7] dark:border-[#3A3A3C] text-[14px] text-[#6E6E73] hover:bg-[#F5F5F7] dark:hover:bg-[#2C2C2E] transition-colors">Annuler</button>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-[12px]">
                ...
                <span className="font-semibold text-[#1D1D1F] dark:text-[#F5F5F7]">
                  {signed?.length ?? 0} / {total}{total > 0 && ` · ${Math.round(((signed?.length ?? 0) / total) * 100)}%`}
                </span>
              </div>
              {total > 0 && (
                <div className="h-1.5 w-full bg-[#F2F2F7] dark:bg-[#3A3A3C] rounded-full overflow-hidden">
                  <div className="h-full bg-green-500 rounded-full transition-all"
                    style={{ width: `${Math.round(((signed?.length ?? 0) / total) * 100)}%` }} />
                </div>
              )}
            </div>
          )}
...
export function DocumentsPageClient({
  ...
  const [libDocs,        setLibDocs]        = useState<DocumentRow[]>(initialLibrary);
  ...
  const filteredMine = useMemo(() => {
    ...
    return list;
  }, [myDocs, search, sizeSort, filterCreator, filterDepartment]);
  ...
  const paginated   = pageSize === 0 ? activeFiltered : activeFiltered.slice((page - 1) * pageSize, page * pageSize);

  async function refreshLibrary() {
    const res = await fetch("/api/admin/documents");
    if (res.ok) setLibDocs(await res.json());
  }
  ...
  async function handleDelete(doc: DocumentRow) {
    ...
    setLibDocs((prev) => prev.filter((d) => d.id !== doc.id));
    setDeleting(null);
  }
  ...
  return (
    <>
      ...
      {/* ── En-tête : titre + bouton upload (même position que "Ajouter un cours") ── */}
    ...
    <div className="space-y-5">
      ...
      {/* ── Row 1: barre d'outils (même structure que cours) ── */}
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {/* Tri taille — toggle identique au tri Durée dans cours */}
        <button
        ...
        {departments.length > 0 && (
          <select
            ...
            onChange={(e) => { setFilterDepartment(e.target.value); resetPage(); }}
      ...
      {/* ── Row 2: onglets sous la barre (si accès bibliothèque) — même style que cours ── */}
      {hasLibrary && (
        <div className="flex border-b border-[#E5E5EA] dark:border-[#3A3A3C]">
          {([
            { key: "mine"    as const, label: "Mes documents à signer", count: myDocs.length    },
            { key: "library" as const, label: "Bibliothèque GRC",       count: libDocs.length   },
          ]).map(({ key, label, count }) => (
            <button
              key={key}
            ...
            >
              ...
              <span className={cn(
                ...
                activeTab === key
                  ...
                  : "bg-[#F5F5F7] dark:bg-[#2C2C2E] text-[#ADADB8]"
              )}>
                {count}
              </span>
            </button>
      ...
      {/* ── Row 3: chips de filtre statut (onglet "mine" seulement) ── */}
      {activeTab === "mine" && (
        <div className="flex gap-1.5 bg-white dark:bg-[#2C2C2E] border border-[#D2D2D7] dark:border-[#3A3A3C] rounded-xl p-1 w-fit">
          {([
            ...
            >
              {dot && <span className={cn("w-2 h-2 rounded-full shrink-0", filterStatus === key ? "bg-white" : dot)} />}
              {label}
            </button>
          ))}
        </div>
      ...
      {activeFiltered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[15px] font-medium text-[#1D1D1F] dark:text-[#F5F5F7]">
            {activeTab === "mine" && myDocs.length === 0
              ? "Aucun document assigné"
              : activeTab === "library" && libDocs.length === 0
              ? "Aucun document PDF"
              : "Aucun résultat"}
          </p>
          <p className="text-[13px] text-[#6E6E73] dark:text-[#8E8E93] mt-1">
            {activeTab === "mine" && myDocs.length === 0
              ? "Vous n'avez pas de document à signer pour l'instant."
              : activeTab === "library" && libDocs.length === 0
              ? canUpload ? "Uploadez votre premier document pour commencer." : "Aucun document dans votre périmètre."
              : "Modifiez votre recherche ou vos filtres."}
          </p>
          {activeTab === "library" && libDocs.length === 0 && canUpload && (
            <button onClick={() => setShowUpload(true)} className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-[#0071E3] hover:bg-[#0077ED] text-white text-[14px] font-medium rounded-xl transition-colors">
      ...
      {/* ── Onglet "Mes documents à signer" — sections par statut ── */}
      {activeTab === "mine" && activeFiltered.length > 0 && (
        <div className="space-y-8">
          {MINE_GROUPS.map(({ key, label, color }) => {
            if (filterStatus !== "all" && filterStatus !== key) return null;
            const group = (paginated as MyDoc[]).filter((d) => d.status === key);
            if (group.length === 0) return null;
            const totalInGroup = filteredMine.filter((d) => d.status === key).length;
            return (
              <div key={key}>
                <div className="flex items-center gap-2.5 mb-3">
                  <div className={cn("w-2.5 h-2.5 rounded-full", color)} />
                  <h2 className="text-[15px] font-semibold text-[#1D1D1F] dark:text-[#F5F5F7]">{label}</h2>
                  <span className="text-[12px] text-[#ADADB8] font-medium">{totalInGroup}</span>
                </div>
                  ...
                  {group.map((doc) => (
                    <div key={doc.id} className="group bg-white dark:bg-[#1C1C1E] rounded-2xl border border-[#E5E5EA] dark:border-[#3A3A3C] overflow-hidden hover:shadow-md hover:border-[#D2D2D7] dark:hover:border-[#636366] transition-all flex flex-col">
                      ...
                      <div className="p-5 flex flex-col gap-3 flex-1">
                        <h3 className="text-[15px] font-semibold text-[#1D1D1F] dark:text-[#F5F5F7] leading-snug line-clamp-2">{doc.title}</h3>
                        {doc.teamName && (
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {doc.dueDate && (
                            <span className="inline-flex items-center gap-1.5 text-[12px] font-medium text-[#6E6E73] dark:text-[#8E8E93] bg-[#F5F5F7] dark:bg-[#2C2C2E] rounded-lg px-2.5 py-1">
                              <Clock className="w-3 h-3" />Échéance {formatDateLong(doc.dueDate)}
                            </span>
                          )}
                        </div>
                        {(doc.createdByName || doc.assignedByName) && (
                          <p className="text-[11px] text-[#ADADB8] leading-relaxed">
                            {doc.createdByName && <span>Créé par <span className="text-[#6E6E73] dark:text-[#8E8E93]">{doc.createdByName}</span></span>}
                            {doc.createdByName && doc.assignedByName && <span> · </span>}
                            {doc.assignedByName && <span>Affecté par <span className="text-[#6E6E73] dark:text-[#8E8E93]">{doc.assignedByName}</span></span>}
                          </p>
                        )}
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-[#F5F5F7] dark:border-[#2C2C2E]">
                          ...
                          <div className="flex items-center gap-1.5">
                            {doc.certificateId && key === "signed" && (
                              <Link href={`/dashboard/certificates/${doc.certificateId}`}
      ...
      {/* ── Onglet "Bibliothèque GRC" — grille de cartes ── */}
      {activeTab === "library" && activeFiltered.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {(paginated as DocumentRow[]).map((doc) => {
            return (
              <div key={doc.id} className="group bg-white dark:bg-[#1C1C1E] rounded-2xl border border-[#E5E5EA] dark:border-[#3A3A3C] overflow-hidden hover:shadow-md hover:border-[#D2D2D7] dark:hover:border-[#636366] transition-all flex flex-col">
                <div className="h-36 overflow-hidden">
                  <PdfThumbnail docId={doc.id} />
                ...
                <div className="p-4 flex flex-col gap-3 flex-1">
                  <h3 className="text-[14px] font-semibold text-[#1D1D1F] dark:text-[#F5F5F7] leading-snug line-clamp-2">{doc.title}</h3>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-[#F5F5F7] dark:border-[#2C2C2E]">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => setAssignDoc(doc)}
                        className="p-2 rounded-lg text-[#6E6E73] dark:text-[#8E8E93] hover:bg-[#F5F5F7] dark:hover:bg-[#2C2C2E] hover:text-[#0071E3] transition-colors" title="Assigner">
                        <Users className="w-3.5 h-3.5" />
                      </button>
                      {(isAdmin || doc.createdById === currentUserId) && (
                        <button onClick={() => setEditDoc(doc)}
                          className="p-2 rounded-lg text-[#6E6E73] dark:text-[#8E8E93] hover:bg-[#F5F5F7] dark:hover:bg-[#2C2C2E] hover:text-[#0071E3] transition-colors" title="Modifier">
                          <Pencil className="w-3.5 h-3.5" />
                        </button>
                      )}
                      <button onClick={() => handleDelete(doc)} disabled={deleting === doc.id}
                        className="p-2 rounded-lg text-[#6E6E73] dark:text-[#8E8E93] hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-500 transition-colors disabled:opacity-40" title="Supprimer">
