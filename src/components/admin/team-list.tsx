...
import { Plus, Pencil, Trash2, Users, ChevronDown, ChevronUp, X, Check, Crown, UserMinus, UserPlus, ArrowUpDown, Upload } from "lucide-react";
...
export function TeamList({ initialTeams, allUsers }: TeamListProps) {
  ...
  const totalPages = pageSize === 0 ? 1 : Math.max(1, Math.ceil(teams.length / pageSize));
    ...
    <div className="space-y-4">
      ...
      {creating ? (
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 h-10 px-4 rounded-xl border border-[#D2D2D7] dark:border-[#3A3A3C] text-[14px] font-medium text-[#3C3C43] dark:text-[#AEAEB2] hover:bg-[#F5F5F7] dark:hover:bg-[#2C2C2E] transition-colors"
          >
      ...
      {teams.length === 0 && !creating && (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-[15px] font-medium text-[#1D1D1F] dark:text-[#F5F5F7]">{t("noTeams")}</p>
          <p className="text-[13px] text-[#6E6E73] dark:text-[#8E8E93] mt-1">{t("createFirst")}</p>
        </div>
      )}
      ...
      {teams.length > 1 && (
        <div className="flex items-center gap-2 flex-wrap">
          {([["name", t("rename")], ["members", t("members")]] as [SortField, string][]).map(([field, lbl]) => (
            <button
              key={field}
              onClick={() => { toggleSort(field); resetPage(); }}
              className={cn(
          ...
          <select
            ...
            className="h-8 px-3 rounded-xl border border-[#D2D2D7] dark:border-[#3A3A3C] bg-white dark:bg-[#2C2C2E] text-[13px] text-[#1D1D1F] dark:text-[#F5F5F7] outline-none focus:border-[#0071E3] transition-all cursor-pointer ml-auto"
          >
            {PAGE_SIZES.map((s) => (
        ...
        {paginated.map((team) => {
          ...
          return (
            <div key={team.id} className="bg-white dark:bg-[#1C1C1E] rounded-2xl border border-[#E5E5EA] dark:border-[#3A3A3C] overflow-hidden">
              ...
              <div className="flex items-center gap-3 px-5 py-4">
                ...
                {editingId === team.id ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[15px] font-semibold text-[#1D1D1F] dark:text-[#F5F5F7]">{team.name}</p>
              ...
              {/* Panneau étendu */}
              {isOpen && (
                <div className="border-t border-[#F5F5F7] dark:border-[#3A3A3C] px-5 py-4 space-y-5 bg-[#FAFAFA] dark:bg-[#2C2C2E]">

                  {/* Manager */}
                  <div className="space-y-2">
                    <p className="text-[12px] font-semibold text-[#6E6E73] dark:text-[#8E8E93] uppercase tracking-wide">{t("manager")}</p>
                    <div className="flex items-center gap-2">
                      <select
                        value={team.manager?.id ?? ""}
                        onChange={(e) => handleSetManager(team.id, e.target.value || null)}
                        className="flex-1 h-9 px-3 rounded-xl border border-[#D2D2D7] dark:border-[#3A3A3C] bg-white dark:bg-[#1C1C1E] text-[13px] text-[#1D1D1F] dark:text-[#F5F5F7] outline-none focus:border-[#0071E3] transition-all"
                      >
                  ...
                  {/* Membres */}
                  <div className="space-y-2">
                    <p className="text-[12px] font-semibold text-[#6E6E73] dark:text-[#8E8E93] uppercase tracking-wide">{t("members")}</p>
      ...
      {pageSize > 0 && totalPages > 1 && (
        <div className="flex items-center justify-between pt-1">
          <p className="text-[13px] text-[#6E6E73] dark:text-[#8E8E93]">
            {(page - 1) * pageSize + 1}–{Math.min(page * pageSize, teams.length)} sur {teams.length}
          </p>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-[#1C1C1E] rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[15px] font-semibold text-[#1D1D1F] dark:text-[#F5F5F7]">{t("deleteTeam")}</p>
                <p className="text-[13px] text-[#6E6E73] dark:text-[#8E8E93]">{deleteTarget.name}</p>
              </div>
            </div>
            <p className="text-[13px] text-[#6E6E73] dark:text-[#8E8E93]">
              {t("deleteTeamDesc")}
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setDeleteTarget(null)} className="flex-1 h-10 rounded-xl border border-[#D2D2D7] dark:border-[#3A3A3C] text-[14px] font-medium text-[#1D1D1F] dark:text-[#F5F5F7] hover:bg-[#F5F5F7] dark:hover:bg-[#2C2C2E] transition-colors">
                {t("cancel")}
              </button>
